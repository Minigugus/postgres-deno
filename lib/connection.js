import { StartupMessage, Close, Execute, auth, Query, Parse, Bind } from './frontend.js'
...
export default function Connection(options = {}) {
  ...
  async function onready(err) {
    ...
    if (!open) {
      if (multi())
        return
      const p = Promise.all(messages.map(socket.write))
      messages = []
      open = true
      await p
    }
...
function postgresSocket(options, {
  ...
  function write() {
    const promise = buffer === null || socket === null ? Promise.resolve() : socket.write(buffer)
