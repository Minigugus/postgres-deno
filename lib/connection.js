...
import { concat, consume, connectUnixSocket, connectTCP } from './util.js'
...
export default function Connection(options = {}) {
  ...
  const socket = postgresSocket(options, {
    ready: () => socket.write(frontConnect(options)),
    data,
    error,
    close
  })
  ...
  function end() {
    ...
    setTimeout(() => ready && ended(), 0) // FIXME : Better replacement for `process.nextTick`
  ...
  async function data(x) {
    buffer = buffer.byteLength === 0
      ? x
      : concat([buffer, x], buffer.length + x.length)
    ...
    if (buffer.byteLength !== 0) // `buffer` may be a zero-copy buffer, so remaining data must be copied
...
function postgresSocket(options, {
  ...
  const x = {
    write: async x => {
      ...
      await write() // Missing "setImmediate" implementation
    },
    destroy: () => {
      socket && socket.close()
      return Promise.resolve()
      ...
      return new Promise(r => socket ? (socket.close(), r()) : r())
    },
    connect
  }

  function write() {
    const promise = socket.write(buffer)
