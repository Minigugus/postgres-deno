...
import {toString, getEnv, readFile, getNodeStreams} from './deno.js'
...
function Postgres(a, b) {
  ...
  function send(connection, query, xs, args) {
    ...
    queueMicrotask(connection.send.bind(null, query, query.tagged ? parseTagged(query, xs, args) : parseUnsafe(query, xs, args)))
  }
  ...
  async function readable(promise, query) {
    const { Readable } = await getNodeStreams();
    ...
    promise.catch(err => query.readable.cancel(err)).then(() => {
      query.connection.blocked = false
      read()
      next()
    })
    return query.readable = new Readable({ read })
  }

  async function writable(promise, query) {
    const { Duplex } = await getNodeStreams();
    ...
    return query.readable = new Duplex({
...
function parseOptions(a, b) {
  const env = getEnv() // eslint-disable-line
      ...
      , user = o.user || o.username || auth[0] || env('PGUSERNAME') || env('PGUSER') || osUsername(env)
  ...
  return Object.assign({
    ...
    target_session_attrs: o.target_session_attrs || url.searchParams.get('target_session_attrs') || env('PGTARGETSESSIONATTRS'),
    debug           : o.debug,
...
function parseUrl(url) {
  if (typeof url !== 'string')
    return { url: new URL('postgres://'), multihost: '' }
...
function osUsername(env) {
  ...
  if (!user)
    throw errors.generic({ code: 'MISSING_USERNAME', message: 'No username provided or it cannot be infered (no environment variable access)' })
  return user
}
