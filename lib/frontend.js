...
async function SASLContinue(type, x, options, pass) {
  ...
  options.serverSignature = await hmacSign(await hmacSign(saltedPassword, fromString('Server Key')), auth, 'utf8', 'base64')

  return bytes.p()
    .str('c=biws,r=' + res.r + ',p=' + await base64Encode(xor(clientKey, await hmacSign(await crypto.subtle.digest("SHA-256", clientKey), auth))))
    .end()
}
