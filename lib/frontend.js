...
import { decodeBase64, concat, toString, md5, fromString, encodeBase64 } from './util.js'
...
function SASL(type, x, options) {
  ...
  options.nonce = decodeBase64(crypto.getRandomValues(new Uint8Array(18)))
...
async function SASLContinue(type, x, options, pass) {
  ...
  const clientKey = await importKey(
    'raw',
  ...
  const exportedClientKey = await exportKey(
    'raw',
    clientKey
  )
  ...
  const serverKey = await importKey(
    'raw',
