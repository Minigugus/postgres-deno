...
import { concat, toString, md5, fromString, encodeBase64, randomBase64, base64Encode, base64Decode } from './deno.js'
...
async function SASLContinue(type, x, options, pass) {
  ...
  const clientKey = hmac('sha256', saltedPassword, fromString('Client Key'));

  const auth = 'n=*,r=' + options.nonce + ','
             + 'r=' + res.r + ',s=' + res.s + ',i=' + res.i
             + ',c=biws,r=' + res.r

  options.serverSignature = hmac('sha256',hmac('sha256', saltedPassword, fromString('Server Key')), auth, 'utf8', 'base64')

  return bytes.p()
    .str('c=biws,r=' + res.r + ',p=' + await base64Encode(xor(clientKey, hmac('sha256', await crypto.subtle.digest("SHA-256", clientKey), auth))))
    .end()
}
