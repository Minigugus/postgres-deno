 ...
 * @returns {Promise<Uint8Array>}
 */
export async function hmacSign(hash, key, msg, inputEncoding, outputEncoding) {
// Force lazy loading for users not using MD5
  const HMAC_URL = 'https://deno.land/x/hmac@v2.0.1/mod.ts';
  // @ts-ignore
  const { hmac } = await import(HMAC_URL);
  return hmac(hash, key, msg, inputEncoding, outputEncoding);
}

/**
 * @param {"md2" | "md4" | "md5" | "ripemd160" | "ripemd160" | "sha1" | "sha224" | "sha256" | "sha384" | "sha512" | "sha3-224" | "sha3-256" | "sha3-384" | "sha3-512" | "keccak224" | "keccak256" | "keccak384" | "keccak512"} hash
 ...
 * @returns {Promise<Uint8Array>}
 */
export async function pbkdf2Derive(hash, password, salt, iterations, keyLen) {
  const PBKDF2_URL = 'https://deno.land/x/crypto@v0.10.0/pbkdf2.ts';
  // @ts-ignore
  const { pbkdf2 } = await import(PBKDF2_URL);
  return pbkdf2(hash, password, salt, iterations, keyLen);
}
